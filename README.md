...
SimpleAIAPI 让内部系统可以复用现有的 OpenAI 风格接口，而调用方只需提供一个 **`model/system/数字键`** 的扁平 JSON。项目同时支持文本、多张图片、LaTeX 渲染、托管页面分享与自动清理图片，是自建模型网关或多团队共用 API Key，又或是手机等不方便运行复杂程序设备进行AI请求和使用的轻量化方案。
